...
def menu(request):
    ...
    if isActive == "1":
        ...
        if dtmfDigits == "2":
            ...
            response += "<Response>"
            response += (
                "<Redirect>http:#62.12.117.25/MF-Ussd-Live/voiceCall.php</Redirect>"
            )
            response += "</Response>"
        ...
        user_create = account.objects.get_or_create(amount=amount, phonenumber=callerNumber)
        user_create.save()
        user_session = session_levels.objects.get_or_create(session_id=sessionId, phonenumber=callerNumber)
        user_session.save()
        user_finance = Microfinance.objects.get_or_create(phonenumber=callerNumber)
        user_finance.save()
